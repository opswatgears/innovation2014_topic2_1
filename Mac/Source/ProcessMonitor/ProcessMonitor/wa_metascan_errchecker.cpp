...
void WaMetascanErrChecker::pushInternalErr( WaWorker* p_des_worker, std::string err_msg )
{
    WaJson* p_err_json = new WaJson( WAAPI_JSON_OBJECT );
    p_err_json->put( L"err", WaStringUtils::_T( ( char* )err_msg.c_str() ).c_str() );
    p_err_json->put( L"type", L"local" );
    p_des_worker->signal( ( void* )p_err_json );
}

void WaMetascanErrChecker::pushServerErr( WaWorker* p_des_worker, std::string& content )
{
    std::string err_msg;
    size_t i_start = content.find( "<title>" );
    if ( i_start != std::string::npos )
    {
        i_start += 7;
        size_t i_end = content.find( "</title>", i_start );
