...
public:
	...
	/**
	 *	@brief Wrapper for keys of type string for operator[]( const wa_wchar * key )
	 *	@see operator[]( const wa_wchar * key )
	 */
	virtual WaJson & operator[]( const wa_wstring & key );
	/**
	 *	@brief Const wrapper for keys of type string for operator[]( const wa_wchar * key ) const
	 *	@see operator[]( const wa_wchar * key ) const
	 */
	...
	/**
	 *	@brief Will clear the internal structure and make this @JSON value @ref WAAPI_JSON_NULL "null"
	...
	/**
	 *	@brief Will put a @JSON value into the type @ref WAAPI_JSON_OBJECT for the given key. This is the string wrapper on the const wa_wchar * version
	 *	@see put( const wa_wchar * key, const WaJson & json_value )
	 */
	...
	/**
	 *	@brief Will get a @JSON value from the type @ref WAAPI_JSON_OBJECT instance for the given key. This is the string wrapper on the const wa_wchar * version
	 *	@see get( const wa_wchar * key )
	 */
	 ...
	 */
	virtual WAUTIL_RETURN get( const wa_wchar * key, wa_wstring & val ) const;

	/**
	 *	@brief will retrieve the string value for the given key if it exists. Useful for known queries. This is a string wrapper on the const wa_wchar * version
	 *	@see get( const wa_wchar * key, wa_wstring & val )
	 */
	virtual WAUTIL_RETURN get( const wa_wstring & key, wa_wstring & val ) const;
	 ...
	 */
	virtual WAUTIL_RETURN get( const wa_wchar * key, wa_int & val ) const;

	/**
	 *	@brief will retrieve the int value for the given key if it exists. Useful for known queries. This is a string wrapper on the const wa_wchar * version
	 *	@see get( const wa_wchar * key, wa_int & val )
	 */
	virtual WAUTIL_RETURN get( const wa_wstring & key, wa_int & val ) const;
	 ...
	 */
	virtual WAUTIL_RETURN get( const wa_wchar * key, bool & val ) const;

	/**
	 *	@brief will retrieve the bool value for the given key if it exists. Useful for known queries. This is a string wrapper on the const wa_wchar * version
	 *	@see get( const wa_wchar * key, bool & val )
	 */
	virtual WAUTIL_RETURN get( const wa_wstring & key, bool & val ) const;
	 ...
	 */
	virtual WAUTIL_RETURN get( const wa_wchar * key, WaJson & val ) const;

	/**
	 *	@brief will retrieve the json value for the given key if it exists. Useful for known queries. This is a string wrapper on the const wa_wchar * version
	 *	@see get( const wa_wchar * key, wa_int & val )
	 */
	virtual WAUTIL_RETURN get( const wa_wstring & key, WaJson & val ) const;

	/**
	 *	@brief Will retrieve the json value for the given key if it exists. This implementation uses dot ('.') notation to allow getting an element from
	 *		an object, potentially many levels deep, in a single query. If a type @ref WAAPI_JSON_ARRAY is in the key chain then the next key
	 *		in the key chain MUST be a numeric index in the array referenced with '@'. Ex: "parent.childArr@3.childElem"
	 *		NOTE: Since this is a get operation, the last (target) element in the chain MUST be an @ref WAAPI_JSON_OBJECT type.
	 *	@param[in] key The key to lookup the string value. An example with dot notation would be "parent.child1.child2"
	 *	@param[out] val The json value referenced by the key
	 *	@retval WAAPI_OK If successfully retrieves the int value for the given key
	 *	@retval WAAPI_ERROR_NOT_FOUND If some key in the key chain does not exist
	 *	@retval WAAPI_ERROR_INVALID_SYNTAX There is an error in the key. This could be due to the end of the chain being the wrong type or an array index out of range.
	 */
	virtual WAUTIL_RETURN dotGet( const wa_wchar * key, WaJson & val );

	/**
	 *	@brief will retrieve the json value for the given key if it exists. This is a string wrapper on the const wa_wchar * version.
	 *	@see dotGet( const wa_wchar * key, WaJson & val )
	 */
	virtual WAUTIL_RETURN dotGet( const wa_wstring & key, WaJson & val );

	/**
	 *	@brief will retrieve the value for the given key if it exists. These are wrappers for retrieving individual types directly.
	 *	@retval WAAPI_ERROR_INVALID_STATE The output type of val did not match the found value.
	 *	@see dotGet( const wa_wchar * key, WaJson & val )
	 */
	virtual WAUTIL_RETURN dotGet( const wa_wchar * key, wa_wstring & val );
	virtual WAUTIL_RETURN dotGet( const wa_wstring & key, wa_wstring & val );
	virtual WAUTIL_RETURN dotGet( const wa_wchar * key, wa_int & val );
	virtual WAUTIL_RETURN dotGet( const wa_wstring & key, wa_int & val );
	virtual WAUTIL_RETURN dotGet( const wa_wchar * key, bool & val );
	virtual WAUTIL_RETURN dotGet( const wa_wstring & key, bool & val );

	/**
	 *	@brief Will retrieve the json value for the given key if it exists. This implementation uses dot ('.') notation to allow getting an element from
	 *		an object, potentially many levels deep, in a single query. If a type @ref WAAPI_JSON_ARRAY is in the key chain then the next key
	 *		in the key chain MUST be a numeric index in the array referenced with '@'. Ex: "parent.childArr@3"
	 *		NOTE: Since this is an at operation, the last (target) element in the chain MUST be an @ref WAAPI_JSON_ARRAY type.
	 *	@param[in] key The key to lookup the string value. An example with dot notation would be "parent.child1.child2@2"
	 *	@param[out] val The json value referenced by the key
	 *	@retval WAAPI_OK If successfully retrieves the int value for the given key
	 *	@retval WAAPI_ERROR_NOT_FOUND If some key in the key chain does not exist
	 *	@retval WAAPI_ERROR_INVALID_SYNTAX There is an error in the key. This could be due to the end of the chain being the wrong type or an array index out of range.
	 */
	virtual WAUTIL_RETURN dotAt( const wa_wchar * key, WaJson & val );

	/**
	 *	@brief will retrieve the json value for the given key if it exists. This is a string wrapper on the const wa_wchar * version.
	 *	@see dotAt( const wa_wchar * key, WaJson & val )
	 */
	virtual WAUTIL_RETURN dotAt( const wa_wstring & key, WaJson & val );
	 ...
	 *	@see dotAt( const wa_wchar * key, WaJson & val )
	 */
	virtual WAUTIL_RETURN dotAt( const wa_wchar * key, wa_wstring & val );
	virtual WAUTIL_RETURN dotAt( const wa_wstring & key, wa_wstring & val );
	virtual WAUTIL_RETURN dotAt( const wa_wchar * key, wa_int & val );
	virtual WAUTIL_RETURN dotAt( const wa_wstring & key, wa_int & val );
	virtual WAUTIL_RETURN dotAt( const wa_wchar * key, bool & val );
	virtual WAUTIL_RETURN dotAt( const wa_wstring & key, bool & val );

	/**
	 *	@brief Will put a @JSON value into the type @ref WAAPI_JSON_OBJECT for the given key. This implementation uses dot ('.') notation to allow insertion of an element
	 *		into an object, potentially many levels deep, in a single query. 
	 *	@details All objects in the chain of children must be type @ref WAAPI_JSON_OBJECT or @ref WAAPI_JSON_ARRAY. If a type @ref WAAPI_JSON_ARRAY
	 *		is in the key chain then the next key in the key chain MUST be a numeric index in the array referenced with '@' Ex: "parent.childArr@3.childElem". To add an element to the
	 *		end of an array use -1 for the index.
	 *		Any elements in the chain that do not exist will be created in the appropriate type.
	 *		NOTE: Since this is a put operation, the last (target) element in the chain MUST be an @ref WAAPI_JSON_OBJECT type. If your expression ends in an array ref then use dotAdd(...). Additionally, if the
	 *		caller is type @ref WAAPI_JSON_ARRAY the first char in the key must still be '@', e.g. "@2.child".
	 *	@param[in] key The key lookup that is to reference the @c json_value held by the @JSON object
	 *	@param[in] json_value The value to store, referenced by @c key in the @JSON object
	 *	@retval WAAPI_ERROR_INVALID_SYNTAX There is an error in the key. This could be due to the end of the chain being the wrong type or an array index out of range.
	 */
	virtual WAUTIL_RETURN dotPut( const wa_wchar * key, const WaJson & val );

	/**
	 *	@brief Will put a @JSON value into the type @ref WAAPI_JSON_OBJECT for the given key. This is a string wrapper on the const wa_wchar * version.
	 *	@see dotPut( const wa_wchar * key, const WaJson & val )
	 */
	virtual WAUTIL_RETURN dotPut( const wa_wstring & key, const WaJson & val );

	/**
	 *	@brief Will put a @JSON value into the type @ref WAAPI_JSON_ARRAY for the given key. This implementation uses dot ('.') notation to allow insertion of an element
	 *		into an object, potentially many levels deep, in a single query. 
	 *	@details All objects in the chain of children must be type @ref WAAPI_JSON_OBJECT or @ref WAAPI_JSON_ARRAY. The type @ref WAAPI_JSON_ARRAY
	 *		in the key chain MUST have a numeric index referenced with '@' Ex: "parent.childArr@3". To add an element to the
	 *		end of an array use -1 for the index.
	 *		Any elements in the chain that do not exist will be created in the appropriate type.
	 *		NOTE: Since this is an add operation, the last (target) element in the chain MUST be an @ref WAAPI_JSON_ARRAY type. If your expression ends in an array ref then use dotAdd(...). Additionally, if the
	 *		caller is type @ref WAAPI_JSON_ARRAY the first char in the key must still be '@', e.g. "@2.child@1".
	 *	@param[in] key The key lookup that is to reference the @c json_value held by the @JSON object
	 *	@param[in] json_value The value to store, referenced by @c key in the @JSON object
	 *	@retval WAAPI_ERROR_INVALID_SYNTAX There is an error in the key. This could be due to the end of the chain being the wrong type or an array index out of range.
	 */
	virtual WAUTIL_RETURN dotAdd( const wa_wchar * key, const WaJson & val );

	/**
	 *	@brief Will put a @JSON value into the type @ref WAAPI_JSON_ARRAY for the given key. This is a string wrapper on the const wa_wchar * version.
	 *	@see dotPut( const wa_wchar * key, const WaJson & val )
	 */
	virtual WAUTIL_RETURN dotAdd( const wa_wstring & key, const WaJson & val );
	...
	/**
	 *	@brief Will remove a @JSON value from the instance if it is of type @ref WAAPI_JSON_OBJECT for the given key. This is a string wrapper on the const wa_wchar * version
	 *	@see remove( const wa_wchar * key )
	 */
	 ...
	 */
	virtual WAUTIL_RETURN val( wa_int & json_value ) const;
	 ...
	 *	@retval WAAPI_ERROR_INVALID_JSON If type does not match this data access 
	 */
	virtual WAUTIL_RETURN val( wa_wstring & json_value ) const;
	 ...
	 *	@retval WAAPI_ERROR_INVALID_JSON If type does not match this data access 
	 */
	virtual WAUTIL_RETURN val( bool & json_value ) const;
...
private:
	...
	/*
	*	This is a helper to perform all the business logic for dotAdd and dotPut.
	*	See those methods for details.
	*/
	virtual WAUTIL_RETURN _dotInsert( const wa_wchar * key, const WaJson & val, const WaJsonType type );

	/*
	*	This is a helper to perform all the business logic for dotGet and dotAt.
	*	See those methods for details.
	*/
	virtual WAUTIL_RETURN _dotRetrieve( const wa_wchar * key, WaJson & val, const WaJsonType type );
};
